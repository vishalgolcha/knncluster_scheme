...
(define reader (cdr (file->lines inputFile)))
(define fline  (car (file->lines inputFile)))
...
;check for file length -1 once again 
...
; pass dc_len in bck
;flist param would be input.txt extracted here ;
...
(define  step2inp (cdr (line_extr inputFile )) )  
...
(define (list-inside? ls val )
  (if (null? ls) #f 
    ;else begins 
    (if 
        (= (car ls) val) #t
        (list-inside? (cdr ls) val)
    )
  )
)

(define ( similar_lists ls1 ls2)
  (if (null? ls1)'()
      ;else begins
      (if (list-inside? ls2 (car ls1) ) 
        (cons (car ls1) (similar_lists (cdr ls1) ls2))
        (similar_lists (cdr ls1) ls2)
    )
  )
)
...
; defining a remover that removes all instances of a number from list
(define (rremove elem list) (filter (lambda (x) (not (equal? x elem))) list))
;defining a list iterator recirsive cars basically
...
(define (send_edge to from lst) (if (list-inside? lst from ) (list to (length(similar_lists (iter step3 from) (iter step3 to))) ) (list '-1)))
...
(define (modif fwd )(sort_sec_r (rremove (list'-1) (generator fwd))))
...
;-----------------------------------------------------step6 ends step 7 not implemented
...
;----------------------------------------------------step9 ends 10 not implemented yet